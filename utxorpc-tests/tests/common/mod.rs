use pallas_addresses::{Address, Network, PaymentKeyHash, ShelleyAddress, StakeKeyHash};
...
use pallas_primitives::conway::{
    AssetName, NonEmptySet, PolicyId, PositiveCoin, PostAlonzoTransactionOutput, TransactionBody,
    TransactionInput, TransactionOutput, Tx, VKeyWitness, Value, WitnessSet,
};
...
pub fn derive_address_from_mnemonic(mnemonic: &str) -> Result<String> {
    ...
    address
        .to_bech32()
        .map_err(|e| Error::ParseError(format!("Address encoding error: {:?}", e)))
}
...
async fn build_transaction_with_utxo(
    ...
    let tx_bytes =
        hex::decode(tx_hash).map_err(|e| Error::ParseError(format!("Invalid tx hash: {:?}", e)))?;
    ...
    let change_has_assets = !input_assets.is_empty()
        && (asset_policy_id.is_none()
            || asset_name.is_none()
            || asset_amount.is_none()
            || input_assets.iter().any(|(p, n, a)| {
                p != asset_policy_id.unwrap()
                    || n != asset_name.unwrap()
                    || *a > asset_amount.unwrap()
            }));
    ...
    let change_output = if change_amount > 0 || !input_assets.is_empty() {
        ...
        if let (Some(policy_id), Some(name), Some(amt)) =
            (asset_policy_id, asset_name, asset_amount)
        {
        ...
        let change_value = if remaining_assets.is_empty() {
            ...
            let mut asset_groups: std::collections::HashMap<
                String,
                Vec<(AssetName, PositiveCoin)>,
...
pub async fn build_transaction(
    ...
    let suitable_utxo = if asset_policy_id.is_some() && asset_name.is_some() {
            ...
            .find(|(_, _, lovelace, assets)| {
                *lovelace >= min_ada
                    && assets.iter().any(|(policy, name, _)| {
                        policy == asset_policy_id.unwrap() && name == asset_name.unwrap()
                    })
            })
            .ok_or_else(|| {
        ...
        utxos
            ...
            .ok_or_else(|| {
                Error::ParseError(format!("No UTXO with at least {} lovelace", min_ada))
            })?
    };
